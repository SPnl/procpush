...
Zie hieronder voor een aantal voorbeeld van data die kunnen worden ingeschoten.
...
* Als het voornaam en achternaam veld leeg zijn en het naam veld niet, dan wordt het naam veld gesplitst in voornaam, tussenvoegsel en achternaam, en het naam veld wordt verwijderd.
* Als het huisnummernummer inclusief toevoeging veld niet leeg is, en het huisnummer veld is leeg, dan wordt het huisnummer inclusief toevoeging veld gesplitst in een huisnummer en huisnummer toevoeging veld en het huisnummer inclusief toevoeging veld wordt verwijderd.
...
* Er wordt gekeken of het een Nederlands telefoonnummer betreft. Als dat zo is dan:
    * Telefoonnummers waarvan de 0 aan het begin ontbreekt worden gerepareerd: 612345678 => 0612345678.
...
* Als er geen achternaam is, maak de achternaam dan '.'. Dit is nodig voor de API van Procurios, die accepteerd geen lege achternaam velden.
...
* Maak nu de data geschikt voor de Procurios API. De veldnamen moeten gewijzigd worden naar veldnamen die de Procurios API aan kan, en de data van de velden moet eventueel geschikt gemaakt worden zodat de Procurios API deze kan verwerken. De veldnamen in de Procurios API kunnen verschillen per Procurios omgeving (staging, live) vandaar dat deze mapping niet hard coded kan. Hoe de verschillende velden heten, en welke data ze verwachten is terug te vinden via een Procurios API call. Deze informatie is op te halen met de procapi module: procapi_registration_get_form_schema(). In de procpush module wordt de mapping van binnenkomende velden naar Procurios API velden ingesteld via /admin/config/sp/procpush. 
